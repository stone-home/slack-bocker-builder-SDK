# -*- coding: utf-8 -*-
# @Filename : test_modal_builder
# @Date : 2022-03-08-14-53
# @Project: content-service-chat-assistant


import pytest
from slack_block_builder import ModalBuilder, Formatter, ButtonElement, ColorScheme


class TestBlockBuilder:
    @pytest.fixture(scope="function")
    def build_obj(self):
        return ModalBuilder(title="Modal",
                            submit="Submit",
                            close="Close")

    def test__it__type(self, build_obj):
        assert build_obj.type == "modal"
        assert build_obj.submit.text == "Submit"
        assert build_obj.close.text == "Close"

    @pytest.mark.parametrize("load_test_json_data", ["test_message_data_modal.json"], indirect=True)
